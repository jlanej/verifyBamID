...
BamPileBases::BamPileBases(const char *bamFile, const char *smID, bool ignoreRG, const char *baiFile) : bIgnoreRG(
        ignoreRG) {
    // open BAM File
    if (!inBam.OpenForRead(bamFile)) {
        Logger::gLogger->error("Cannot open BAM file %s for reading - %s", bamFile,
                               SamStatus::getStatusString(inBam.GetStatus()));
    }
    // read header
            ...
            if ((!bIgnoreRG) || (vsRGIDs.size() == 0)) {
                ...
                if (sBamSMID.empty()) {
                    ...
                    Logger::gLogger->warning(
                            "SM is not identical across the readGroups. Ignoring .bestSM/.selfSM outputs");
                    bSameSMFlag = false;
                }
...
int BamPileBases::readMarker(const char *chrom, int position, bool ignoreOverlapPair) {
    ...
    if (mSN2RefID.find(searchChrom) == mSN2RefID.end()) {
        ...
        if (ret.second == true) {  // New entry, so warn.
            Logger::gLogger->warning(
                    "Cannot find sequence name %s appeared in VCF file but not in the BAM file, perhaps the reference is incompatible?\nThese markers will be ignored..",
                    chrom);
        }
